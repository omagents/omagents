...
완료되었습니다. 첫 실행 시 OmAgents가 자동으로 다음을 수행합니다:
...
OmAgents를 로컬 [Claude Code](https://claude.ai/code) 플러그인으로 저장소에서 바로 설치할 수 있습니다:

```bash
claude plugin install /path/to/omagents
```

첫 실행 시 플러그인이 번들된 skills와 MCP servers를 등록합니다. Claude Code는 OpenCode 전용인 `shell.env` PATH injection과 병렬 실행 엔진 대신 `SessionStart` hooks와 `bin/` wrappers를 사용하므로, background task dispatch(`task(background: true)`)와 `/ps` 명령어를 사용할 수 없습니다.

`skills/` 디렉토리의 소스 skill을 편집한 경우, 생성된 플러그인 산출물을 재생성하려면 다음을 실행하세요:
...
OmAgents를 로컬 [Codex](https://github.com/openai/codex) 플러그인으로 저장소에서 바로 설치할 수 있습니다:

```bash
codex plugin add /path/to/omagents
```

Claude Code와 동일한 플랫폼 안내가 적용됩니다: `SessionStart` hooks와 `bin/` wrappers가 OpenCode 전용 `shell.env` PATH injection과 병렬 실행 엔진을 대체하므로 background task dispatch와 `/ps`를 사용할 수 없습니다. 소스 skill을 편집한 후에는 `npm run sync`를 실행하여 `.claude-plugin/`과 `.codex-plugin/` 산출물을 재생성하세요。
