...
**사전 요구 사항:** 저장소 루트에 `.claude-plugin/plugin.json`이 포함되어야 합니다(`npm run sync`로 생성).
...
세션 시작 시 플러그인이 번들된 skills와 MCP servers를 검색하고 활성화합니다. Claude Code는 OpenCode 전용인 `shell.env` PATH injection과 병렬 실행 엔진 대신 `SessionStart` hooks와 `bin/` wrappers를 사용하므로, background task dispatch(`task(background: true)`)와 `/ps` 명령어를 사용할 수 없습니다.
...
**사전 요구 사항:** 저장소 루트에 `.codex-plugin/plugin.json`이 포함되어야 합니다(`npm run sync`로 생성).
