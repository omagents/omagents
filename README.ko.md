...
**사전 요구 사항:** 플러그인 JSON 파일(`.claude-plugin/plugin.json` 및 `.codex-plugin/plugin.json`)은 소스 skill에서 `npm run sync`로 생성되며 저장소에 커밋되어 있습니다. 소스 skill을 편집한 후에는 `npm run sync`를 실행하여 이들을 재생성하세요.
...
`skills/` 디렉토리의 소스 skill을 편집한 경우, `npm run sync`를 다시 실행하여 생성된 플러그인 산출물을 재생성하세요.
