...
mkdir -p "$ROOT_DIR/.codex-plugin/skills"
mkdir -p "$ROOT_DIR/.codex-plugin/hooks"

cp "$ROOT_DIR/scripts/templates/hooks.codex.json" "$ROOT_DIR/.codex-plugin/hooks/hooks.json"
cp "$ROOT_DIR/hooks/setup-venv.sh" "$ROOT_DIR/.codex-plugin/hooks/setup-venv.sh"
chmod +x "$ROOT_DIR/.codex-plugin/hooks/setup-venv.sh"
...
sed -e "s/{{VERSION}}/$VERSION/g" -e "s/{{DESCRIPTION}}/$DESCRIPTION/g" \
  "$SCRIPT_DIR/templates/plugin.codex.json" > "$ROOT_DIR/.codex-plugin/plugin.json"

node - "$ROOT_DIR/mcp-servers/base.json" "$ROOT_DIR/.codex-plugin/.mcp.json" <<'JS'
const fs = require("fs");
...
fs.writeFileSync(process.argv[3], JSON.stringify(out, indent=2) + "\n");
JS

# Copy skills with platform-specific tool mapping and overrides
SKILL_SOURCE_DIR="$ROOT_DIR/skills"
SKILL_TARGET_DIR="$ROOT_DIR/.codex-plugin/skills"
...
my %map = (
PL_HEADER
  while IFS='|' read -r opencode_tool codex_val; do
    ...
    val_escaped=$(printf '%s' "$codex_val" | sed 's/["$@\\]/\\&/g')
...
for skill_dir in "$SKILL_SOURCE_DIR"/*/; do
  ...
  if [[ -f "$skill_dir/SKILL.codex.md" ]]; then
...
echo "[sync] generated Codex plugin"
