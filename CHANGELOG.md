...
- **config.toml duplicate key error**: installer now strips ALL orphaned omagents sections (including from older installs without markers) before writing. Line-by-line scanning replaces fragile regex.
...
- **Codex SessionStart hook JSON output**: `setup-venv.sh` now outputs `{`}` to stdout (Codex requires JSON), with all informational messages redirected to stderr.
