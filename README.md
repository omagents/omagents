...
**Prerequisite:** The repository root must contain `.claude-plugin/plugin.json` (generated by `npm run sync`).

Run OmAgents with the plugin directory:
...
**Prerequisite:** The repository root must contain `.codex-plugin/plugin.json` (generated by `npm run sync`).
