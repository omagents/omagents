...
OpenCodeのpluginで、agent skills、MCP servers、並列実行、superpowersを1つのインストールにまとめて提供します。
...
以上です。初回起動時にOmAgentsは自動的に以下を実行します:
...
OmAgents をローカルの [Claude Code](https://claude.ai/code) プラグインとして、リポジトリから直接インストールできます：

```bash
claude plugin install /path/to/omagents
```

初回起動時に、プラグインはバンドルされた skills および MCP servers を登録します。Claude Code は OpenCode 独自の `shell.env` PATH 注入と並列実行エンジンの代わりに、`SessionStart` hooks と `bin/` wrappers を使用するため、background task の dispatch（`task(background: true)`）や `/ps` コマンドは利用できません。

`skills/` ディレクトリのソース skill を編集した場合、生成されたプラグイン産物を再生成するために以下を実行してください：
...
OmAgents をローカルの [Codex](https://github.com/openai/codex) プラグインとして、リポジトリから直接インストールできます：

```bash
codex plugin add /path/to/omagents
```

Claude Code と同じプラットフォーム上の注意が適用されます：`SessionStart` hooks と `bin/` wrappers が OpenCode 独自の `shell.env` PATH 注入と並列実行エンジンに代わるため、background task の dispatch や `/ps` は利用できません。ソース skill を編集した後は `npm run sync` を実行して `.claude-plugin/` および `.codex-plugin/` 産物を再生成してください。
