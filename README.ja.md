...
**前提条件：** リポジトリルートに `.claude-plugin/plugin.json` が含まれている必要があります（`npm run sync` で生成）。
...
セッション開始時に、プラグインはバンドルされた skills および MCP servers を検出して有効化します。Claude Code は OpenCode 独自の `shell.env` PATH 注入と並列実行エンジンの代わりに、`SessionStart` hooks と `bin/` wrappers を使用するため、background task の dispatch（`task(background: true)`）や `/ps` コマンドは利用できません。
...
**前提条件：** リポジトリルートに `.codex-plugin/plugin.json` が含まれている必要があります（`npm run sync` で生成）。
