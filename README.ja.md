...
### Claude Code（プラグインディレクトリ）

**前提条件：** プラグイン JSON ファイル（`.claude-plugin/plugin.json` と `.codex-plugin/plugin.json`）は、ソース skill から `npm run sync` で生成され、リポジトリにコミットされています。ソース skill を編集した後は `npm run sync` を実行して、これらを再生成してください。

1. プラットフォーム固有のプラグインファイルを生成：

```bash
npm run sync
```

2. プラグインディレクトリで OmAgents を実行：
...
`skills/` ディレクトリのソース skill を編集した場合、`npm run sync` を再実行して生成されたプラグイン産物を再生成してください。
