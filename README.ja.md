...
1. OmAgents marketplace を追加してプラグインをインストール：
...
セッション開始時に、プラグインはバンドルされた skills、MCP servers を自動検出し、`SessionStart` hooks で Python venv をセットアップします。OpenCode 独自の並列実行エンジン（`task(background: true)`、`/ps`）は利用できません。Claude Code のネイティブ `Agent` ツールを使用してください。
...
セッション開始時に、プラグインはバンドルされた skills、MCP servers を自動検出し、`SessionStart` hooks で Python venv をセットアップします。OpenCode 独自の並列実行エンジンは利用できません。Codex のネイティブ subagent ツールを使用してください。

> **開発者向け：** `skills/` のソース skill を編集した場合、`npm run sync` を実行して `.claude-plugin/` および `.codex-plugin/` 産物を再生成し、コミットしてください。CI が生成ファイルの最新状態を自動検証します。
